...
from stable_baselines3.td3 import MlpPolicy, CnnPolicy, MultiInputPolicy
...
def main(args):
    ...
    env = gym.make('ShelfEnvironmentImMIPositioninsURSim-v0', ur_model='ur10e', ip=target_machine_ip, gui=False)
    ...
    model = TD3(MultiInputPolicy, env, buffer_size=2500, verbose=1,
                tensorboard_log="./multiinput_reach_tensorboard/", batch_size=128)

    model.learn(total_timesteps=args['num_episodes'], callback=callback)
    model.save('TD3_MultiInput')
