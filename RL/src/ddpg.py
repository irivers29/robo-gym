...
class Noise(object):
    def __ini__(self, mu, sigma=0.15, theta=0.2, dt=1e-2, x0=None):
        self.theta = theta
        self.mu = mu
        self.sigma = sigma
        self.dt = dt
...
class CriticNetwork(nn.Module):
    def __init__(self, beta, input_dims, fc1_dims, fc2_dims, n_actions, name, chkpt_dir='tmp/ddpg'):
        ...
        f3 = 0.003
        self.q = nn.Linear(self.fc2_dims, 1)            ### This has to be changed
...
class Agent(object):
    ...
    def learn(self):
        ...
        reward = T.tnsor(reward, dtype=T.float).to(self.critic.device)
