...
class URBaseEnv(gym.Env):
    ...
    def add_fixed_joints(self, action) -> np.array:
        ...
        fixed_joints = np.array([self.fix_base, self.fix_shoulder, self.fix_elbow, self.fix_wrist_1, self.fix_wrist_2, self.fix_wrist_3, self.fix_gripper])
    ...
    def env_action_to_rs_action(self, action) -> np.array:
        ...
        # Scale action
        rs_action_arm = np.multiply(rs_action[0:6], self.abs_joint_pos_range[0:6])
        rs_action_grip = self.ur.grip_to_rs_action(rs_action[6])

        rs_action = np.concatenate((rs_action_arm, rs_action_grip))
    ...
    def _set_joint_positions(self, joint_positions) -> None:
        ...
        self.joint_positions['robotiq_85_left_knuckle_joint_position'] = joint_positions[6]
    ...
    def _get_joint_positions_as_array(self) -> np.array:
        ...
        joint_positions.append(self.joint_positions['robotiq_85_left_knuckle_joint_position'])
    ...
    def _get_observation_space(self) -> gym.spaces.Box:
        ...
        min_joint_positions = np.full(7, -1.0)
    ...
    def _get_action_space(self)-> gym.spaces.Box:
        ...
        """
        fixed_joints = [self.fix_base, self.fix_shoulder, self.fix_elbow, self.fix_wrist_1, self.fix_wrist_2, self.fix_wrist_3, self.fix_gripper]
        num_control_joints = len(fixed_joints) - sum(fixed_joints)

        #if not self.fix_gripper:
         #   
          #  low_action = np.full((num_control_joints - 1), -1.0)
           # min_action_grip = np.full(1, 0.0)
            #low = np.concatenate((low_action,min_action_grip))
    ...
    def _on_camera_state(self, msg):
        ...
        self.image_np = cv2.imdecode(np_array, cv2.IMREAD_COLOR)
        #self.image_list = image_np.tolist()

class ShelfEnvironmentURSim(URBaseEnv, Simulation):
...
class ShelfEnvironmentURRob(URBaseEnv):
    real_robot = True
